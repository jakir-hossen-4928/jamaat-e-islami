...
const UpazilaAdminDashboard = () => {
  const { userProfile } = useAuth();

  const { data: stats, isLoading } = useQuery({
    queryKey: ['upazila-admin-stats', userProfile?.accessScope?.upazila_id],
    queryFn: async () => {
      if (!userProfile?.accessScope?.upazila_id) return null;

      const votersRef = collection(db, 'voters');
      const votersQuery = query(votersRef, where('upazila_id', '==', userProfile.accessScope.upazila_id));
    ...
    enabled: !!userProfile?.accessScope?.upazila_id
  });
    ...
    <RoleBasedSidebar>
      <div className="space-y-6">
        <div className="bg-gradient-to-r from-purple-600 to-purple-800 rounded-lg p-6 text-white">
          <h1 className="text-2xl lg:text-3xl font-bold">উপজেলা অ্যাডমিন ড্যাশবোর্ড</h1>
          <p className="mt-2 text-purple-100">আপনার উপজেলার ভোটার ব্যবস্থাপনা</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <Button className="w-full" variant="outline" asChild>
                <a href="/admin/voters">উপজেলার ভোটার দেখুন</a>
              </Button>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <Button className="w-full" variant="outline" asChild>
                <a href="/admin/verify-users">গ্রাম অ্যাডমিন যাচাই</a>
              </Button>
...
export default UpazilaAdminDashboard;
