...
const WardAdminDashboard = () => {
  ...
  const { data: stats, isLoading } = useQuery({
    queryKey: ['ward-admin-stats', userProfile?.accessScope?.ward_id],
    queryFn: async () => {
      if (!userProfile?.accessScope?.ward_id) return null;

      const votersRef = collection(db, 'voters');
      const votersQuery = query(votersRef, where('ward_id', '==', userProfile.accessScope.ward_id));
    ...
    enabled: !!userProfile?.accessScope?.ward_id
  });
    ...
    <RoleBasedSidebar>
      <div className="space-y-6">
        <div className="bg-gradient-to-r from-teal-600 to-teal-800 rounded-lg p-6 text-white">
          <h1 className="text-2xl lg:text-3xl font-bold">ওয়ার্ড অ্যাডমিন ড্যাশবোর্ড</h1>
          <p className="mt-2 text-teal-100">আপনার ওয়ার্ডের ভোটার ব্যবস্থাপনা</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <Button className="w-full" variant="outline" asChild>
                <a href="/admin/voters">ওয়ার্ডের ভোটার দেখুন</a>
              </Button>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle>ওয়ার্ড ব্যবস্থাপনা</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
