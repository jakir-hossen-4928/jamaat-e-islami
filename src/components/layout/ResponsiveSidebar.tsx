...
import { Users, BarChart3, MessageSquare, Database, Plus, LogOut, Home, UserCheck, FileText, Settings, MapPin } from 'lucide-react';
...
const AppSidebar = () => {
  ...
  const getRoleDisplayName = (role: string) => {
    switch (role) {
      ...
      case 'division_admin':
        return 'বিভাগীয় অ্যাডমিন';
      case 'district_admin':
        return 'জেলা অ্যাডমিন';
      case 'upazila_admin':
        return 'উপজেলা অ্যাডমিন';
      case 'union_admin':
        return 'ইউনিয়ন অ্যাডমিন';
      case 'village_admin':
        return 'গ্রাম অ্যাডমিন';
      default:
        return 'ব্যবহারকারী';
    }
  };
  ...
  return <Sidebar className="bg-gray-500 text-white border-r border-gray-400 shadow-xl">
      <SidebarHeader className="bg-gray-600 p-4 border-b border-gray-400">
        <div className="flex items-center space-x-3">
          <img src="https://i.ibb.co/6Rt79ScS/bangladesh-jamaat-e-islami-seeklogo.png" alt="Logo" className="w-8 h-8 rounded-full bg-white p-1" />
      ...
      <SidebarContent className="px-2 py-4 bg-teal-500">
        <SidebarGroup>
      ...
      <SidebarFooter className="p-4 border-t border-gray-400">
        <div className="bg-gray-600 rounded-lg p-4 mb-4 shadow-inner border border-gray-400">
          <div className="space-y-1">
            ...
            {userProfile.accessScope && userProfile.role !== 'super_admin' && <div className="mt-2 text-gray-200 text-xs space-y-0.5 border-t border-gray-400 pt-2">
                {userProfile.accessScope.division_name && <p className="truncate">বিভাগ: {userProfile.accessScope.division_name}</p>}
                {userProfile.accessScope.district_name && <p className="truncate">জেলা: {userProfile.accessScope.district_name}</p>}
                {userProfile.accessScope.upazila_name && <p className="truncate">উপজেলা: {userProfile.accessScope.upazila_name}</p>}
                {userProfile.accessScope.union_name && <p className="truncate">ইউনিয়ন: {userProfile.accessScope.union_name}</p>}
                {userProfile.accessScope.village_name && <p className="truncate">গ্রাম: {userProfile.accessScope.village_name}</p>}
              </div>}
          </div>
        </div>
        <Button onClick={handleLogout} variant="outline" className="w-full text-gray-800 border-gray-200 bg-white hover:bg-gray-100 hover:border-gray-300 transition-colors font-medium">
